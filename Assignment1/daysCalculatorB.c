 ...
 *   line depending on including the last date or not.
 *   Compilation: gcc -ansi -o daysCalculatorB daysCalculatorB.c
 *   Execution: ./daysCalculatorB 2 4 2001 4 2 2001 include
 */
...
int main(int argc, char *argv[]) {
  ...
  /*check if user gives enough info, assign include to the variable */
  if (argc < 7) {
    printf("Usage: ./A dd1 mm1 yyyy1 dd2 mm2 yyyy2 char\n");
  }
  else if (argc == 8){
    dd1 = atoi ( argv[1] );
    mm1 = atoi ( argv[2] );
  ...
  /*check to see if its a leap year or not*/
  ...
  prints out number of days difference*/
  if ((mm1 < 1)||(mm1 > 12)) {
